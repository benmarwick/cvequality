...
The coefficient of variation was introduced by Karl Pearson in [1896](www.jstor.org/stable/90707). Since then it has become widely used in chemistry, engineering, physics, sociology, finance, and other fields. Despite its ubiquity, we lack convienent methods for testing if $c_v$ values from different samples are the equal or not. Numerous statistical methods have been described in publications, but until now, none have been implemented in an easy-to-use statistical package. This package aims to remedy this situation by providing methods for testing for the equality of $c_v$ values from different samples. 

Note that there are some limitations to using $c_v$ values. The main restriction is that $c_v$ should only be computed for data measured on a ratio scale, as these are the measurements that can only take non-negative values. For example it is not appropriate to compare $c_v$ of measurements made using Celcius and Fahrenheit, because these are interval scales that can take negative values and have arbitrary zero points. When the mean of a variable is zero, the $c_v$ cannot be computed, and when the mean is close to zero and the variable contains both positive and negative values, then the $c_v$ can be inaccurate. A lesser concern is that log-normal data have stationary $c_v$ values, and so they are generally uniformative. 
...
To demonstrate the use the functions with raw data we will use Galton's [(1886)](http://www.jstor.org/stable/2841583) observations of heights of 934 children in 205 families. This is the same dataset that Pearson used when he introduced the $c_v$

```{r}
# read in the data, we obtained this from the HistData package
GaltonFamilies <- read.csv("GaltonFamilies.csv")

# Is variation in height is greater for males than females? Let's reshape the
# data so we have a dataframe with one column of observations, and a second
# column of a grouping variable
parent_height_by_gender <- 
  data.frame(height = c(GaltonFamilies$father, # observations
                        GaltonFamilies$mother),
             # grouping variable
             gender = c(rep("male", length(GaltonFamilies$father)),
                        rep("female", length(GaltonFamilies$mother))))

# CVs for each gender
cv_parent_male <- with(GaltonFamilies, sd(father)/mean(father))
cv_parent_female <- with(GaltonFamilies, sd(mother)/mean(mother))
```

We can take a quick look at the data to get some intuition about the variation:

```{r}
library(ggplot2)
library(ggbeeswarm)
ggplot(parent_height_by_gender, 
       aes(gender, 
           height)) +
  geom_boxplot() +
  geom_quasirandom() +
...
# test for difference between fathers and mothers
library(cvequality)
parent_height_by_gender_cv_test <- 
with(parent_height_by_gender, 
     asymptotic_test(height,
                     gender))

```

We see that the $c_v$ for mothers (`r cv_parent_female`) and fathers (`r cv_parent_male`) are very simliar. The Feltz and Miller asymptotic test returns a D'AD statistic value of `r parent_height_by_gender_cv_test$D_AD`, which results in a p-value of `r parent_height_by_gender_cv_test$p_value`. We do not reject the hypothesis that both groups have the same $c_v$. 
...
Galton, F. (1886). Regression Towards Mediocrity in Hereditary. Stature Journal of the Anthropological Institute, 15, 246-263. http://www.jstor.org/stable/2841583

Pearson, K. (1896). Mathematical Contributions to the Theory of Evolution. III. Regression, Heredity, and Panmixia. Philosophical Transactions of the Royal Society of London. Series A, Containing Papers of a Mathematical or Physical Character, 187, 253-318. Retrieved from http://www.jstor.org/stable/90707