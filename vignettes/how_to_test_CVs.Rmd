...
Note that there are some limitations to using $c_v$ values. The main restriction is that $c_v$ should only be computed for data measured on a ratio scale, as these are the measurements that can only take non-negative values. For example it is not appropriate to compare $c_v$ of measurements made using Celcius and Fahrenheit, because these are interval scales that can take negative values. When the mean of a variable is zero, the $c_v$ cannot be computed, and when the mean is close to zero and the variable contains both positive and negative values, then the $c_v$ can be inaccurate. A lesser concern is that log-normal data have stationary $c_v$ values, and so they are generally uniformative. 
